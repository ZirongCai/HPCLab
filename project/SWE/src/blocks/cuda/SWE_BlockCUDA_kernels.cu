...
__global__ 
void initScenario_cuda(
    float *h, float *hu, float *hv, float *b,
    float *h_s, float *hu_s, float *hv_s, float *b_s,
    const int i_nX, const int i_nY)
{
  int l_cellIndexI, l_cellIndexJ;

  l_cellIndexI = 1 + TILE_SIZE*blockIdx.x + threadIdx.x;
  l_cellIndexJ = 1 + TILE_SIZE*blockIdx.y + threadIdx.y;

  int l_cellPosition = getCellCoord(l_cellIndexI, l_cellIndexJ, i_nY);


  h[l_cellPosition] = h_s[l_cellPosition];
  hu[l_cellPosition] = hu_s[l_cellPosition];
  hv[l_cellPosition] = hv_s[l_cellPosition];
  b[l_cellPosition] = b_s[l_cellPosition];
}
