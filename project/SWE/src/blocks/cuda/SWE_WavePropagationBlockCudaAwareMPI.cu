...
//#define PREFETCH
/**
 * Constructor of a SWE_WavePropagationBlockCudaAwareMPI.
 *
 ...
 */
SWE_WavePropagationBlockCudaAwareMPI::SWE_WavePropagationBlockCudaAwareMPI(
    int l_nx, int l_ny,
    float l_dx, float l_dy)
    : SWE_BlockCudaAwareMPI(l_nx, l_ny, l_dx, l_dy)
{
...
/**
 * Destructor of a SWE_WavePropagationBlockCudaAwareMPI.
 *
 ...
 */
__host__
void SWE_WavePropagationBlockCudaAwareMPI::simulateTimestep(float i_dT) {
 ...
 */
__host__
float SWE_WavePropagationBlockCudaAwareMPI::simulate(float tStart, float tEnd) {
  float t = tStart;
  do {
 ...
 */
void SWE_WavePropagationBlockCudaAwareMPI::computeNumericalFluxes() {
  /*
   * Initialization.
   */
  ...
  checkCUDAError("allocate device memory for l_maximumWaveSpeedsD");

//#ifdef NON_BLOCKING
//  MPI_Status status[24];
//  MPI_Waitall(24, this->comm_requests, status);
//#endif
  ...
  computeNetUpdatesKernel<<<dimGrid,dimBlock>>>( h.elemVector(), hu.elemVector(), hv.elemVector(), b.elemVector(),
  ...
  computeNetUpdatesKernel<<<dimRightGrid, dimRightBlock>>>( h.elemVector(), hu.elemVector(), hv.elemVector(), b.elemVector(),
      hNetUpdatesLeftD,  hNetUpdatesRightD,
      huNetUpdatesLeftD, huNetUpdatesRightD,
      hNetUpdatesBelowD, hNetUpdatesAboveD,
      hvNetUpdatesBelowD, hvNetUpdatesAboveD,
  ...
  computeNetUpdatesKernel<<<dimTopGrid, dimTopBlock>>>( h.elemVector(), hu.elemVector(), hv.elemVector(), b.elemVector(),
      hNetUpdatesLeftD,  hNetUpdatesRightD,
      huNetUpdatesLeftD, huNetUpdatesRightD,
      hNetUpdatesBelowD, hNetUpdatesAboveD,
      hvNetUpdatesBelowD, hvNetUpdatesAboveD,
  ...
  //set the maximum time step for this SWE_WavePropagationBlockCudaAwareMPI
 ...
 */
void SWE_WavePropagationBlockCudaAwareMPI::updateUnknowns(const float i_deltaT) {
  //! definition of one CUDA-block. Typical size are 8*8 or 16*16
  dim3 dimBlock(TILE_SIZE,TILE_SIZE);
  ...
  updateUnknownsKernel<<<dimGrid,dimBlock>>>( hNetUpdatesLeftD, hNetUpdatesRightD,
      ...
      hNetUpdatesBelowD, hNetUpdatesAboveD,
      hvNetUpdatesBelowD, hvNetUpdatesAboveD,
      h.elemVector(), hu.elemVector(), hv.elemVector(),
