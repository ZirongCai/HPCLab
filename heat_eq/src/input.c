...
int read_input(FILE* infile, algoparam_t* param)
{
    ...
    // Get dimx, dimy from input file
    int dimx, dimy;
    fgets(buf, BUFSIZE, infile);
    n = sscanf(buf, "%u %u", &(param->dimx), &(param->dimy));
    if ((param->dimx) == 0 || (param->dimy) == 0) {
        fprintf(stderr, "Neither dimessions input should be zeros \n");
        return 0;
    }
...
void print_params(algoparam_t* param)
{
    ...
    fprintf(stderr, "Resolutions       : (%u, %u, ... %u)\n", param->initial_res,
            param->initial_res + param->res_step_size, param->max_res);
    fprintf(stderr, "Iterations        : %u\n", param->maxiter);
    fprintf(stderr, "Num. Heat sources : %u\n", param->numsrcs);

    for (i = 0; i < param->numsrcs; i++) {
        fprintf(stderr, "  %2d: (%2.2f, %2.2f) %2.2f %2.2f \n", i + 1, param->heatsrcs[i].posx, param->heatsrcs[i].posy,
                param->heatsrcs[i].range, param->heatsrcs[i].temp);
    }
}
